...
def analyze_svm_results(svm, X_train, y_train, X_test, y_test, image_shape=(32, 32, 3), typeOf="linearSVM"):
    """
    ...
    save_path = os.path.join("images", f"SV_as_image{typeOf}")
    ...
    if svm.kernel == 'linear' and svm.w is not None:
        ...
        plt.axis("off")
        save_path = os.path.join("images", f"weightImage{typeOf}")
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.show()
